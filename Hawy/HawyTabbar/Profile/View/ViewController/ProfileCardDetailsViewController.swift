//
//  ProfileCardDetailsViewController.swift
//  Hawy
//
...
class ProfileCardDetailsViewController: BaseViewViewController, UITextFieldDelegate, BottomPopupDelegate {
    ...
    func performRequest() { //url: String, parameters: [String:Any]
        ...
        let param: [String: Any] = [
            ...
            "name": animalNameTF.text ?? "",
            "mother_name": motherNameTF.text ?? "",
            "father_name": fatherNameTF.text ?? "",
            "age": ageTF.text ?? "",
            "category_id": catID ?? "",
            "notes": noteTF.text ?? "",
            "status": "active",
            "inoculations": orderName,
            "owners": propertyName,
            "documentation_number": docNumTF.text ?? ""
        ...
        AF.request("https://hawy-kw.com/api/auth/profile/cards/show?id=\(id ?? 0)&user_id=\(userId ?? 0)", method: .get, parameters: nil, headers: headers)
            ...
            do {
                ...
                if productResponse.code == 200 {
                    ...
                    DispatchQueue.main.async {
                        self.animalNameTF.text = productResponse.item?.name ?? ""
                        self.typeTF.text = productResponse.item?.categoryName ?? ""
                        self.fatherNameTF.text = productResponse.item?.fatherName ?? ""
                        self.motherNameTF.text = productResponse.item?.motherName ?? ""
                        self.ageTF.text = productResponse.item?.age ?? ""
                        self.docNumTF.text = productResponse.item?.documentation_number ?? ""
                        self.noteTF.text = productResponse.item?.notes ?? ""
                        self.catID = "\(productResponse.item?.categoryID ?? 1)"
                        
                        for owner in productResponse.item?.owners ?? [] {
                            //self.orderName.insert(owner.name ?? "", at: 0)
                            self.addPrope.append(ADDPrope(prope: owner.name ?? ""))
                            
                        }
    
                        for inoculation in productResponse.item?.inoculations ?? [] {
                            //self.propertyName.insert(inoculation.name ?? "", at: 0)
                            self.addVacce.append(ADDVacce(vacc: inoculation.name ?? ""))
                        }
...
//            ToastManager.shared.showError(message: "Please, Enter category", view: self.view)
//            return
//        }
//
...
//                print(self.images)
//
//                let getCatAndSub = try await viewModel.addedCardDetails(name: animalNameTF.text, mother_name: motherNameTF.text, father_name: fatherNameTF.text, age: ageTF.text, main_image: mainImmage, images: images, video: video, category_id: catID, notes: "ddfdf", status: "active", inoculations: orderName, owners: propertyName)
//                print(getCatAndSub)
//
//            }catch {
...
extension ProfileCardDetailsViewController: UITableViewDelegate, UITableViewDataSource {
